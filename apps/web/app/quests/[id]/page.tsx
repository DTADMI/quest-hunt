...
const generateMockQuest = (id: string): Quest => {
    ...
    return {
        ...
        longDescription: 'Embark on this exciting journey through the city, solving puzzles and discovering hidden gems along the way. This quest will take you to some of the most interesting locations, each with its own unique challenge. Perfect for explorers of all ages!',
        difficulty,
...
export default function QuestDetailPage() {
    ...
    const progress = quest ?
        Math.round((quest.waypoints.filter(wp => wp.status === 'completed').length / quest.waypoints.length) * 100) : 0;
        ...
        <div className="container mx-auto px-4 py-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" size="icon" onClick={toggleBookmark}>
                            <Heart
                                className={`h-5 w-5 ${isBookmarked ? 'fill-red-500 text-red-500' : ''}`}
                            />
                        </Button>
              ...
              <span className="font-medium">
                {quest.isCompleted ? 'Completed!' : 'Quest Progress'}
                        ...
                        <p className="text-sm text-muted-foreground">
                            {quest.waypoints.filter(wp => wp.status === 'completed').length} of {quest.waypoints.length} waypoints
                            completed
                        </p>
                    </div>
                )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {quest.waypoints.length - quest.waypoints.filter(wp => wp.status === 'completed').length}
                  </span>
                                )}
                            </TabsTrigger>
                            <TabsTrigger value="reviews">
                                Reviews ({quest.reviewCount})
                            </TabsTrigger>
                        </TabsList>
                                            ...
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="mt-1">
                                                        ...
                                                        Definitely recommend to anyone looking for an adventure.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div>
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        // Scroll to waypoint
                                                        const element = document.getElementById(`waypoint-${activeWaypoint.id}`);
                                                        if (element) {
